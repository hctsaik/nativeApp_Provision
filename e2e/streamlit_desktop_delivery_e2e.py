...
def main() -> int:
    ...
    step(3, "目標機:sentinel 該在的在、該不在的不在")
    ...
    step(4, "目標機:bootstrap --status(電話另一頭念得出來的一頁)")
    out = run_bootstrap(tree, "--status")
    print("    " + "\n    ".join(out.stdout.strip().splitlines()[:12]))
    assert out.returncode == 0, out.stderr

    step(5, "建置 v1.1.0(同一份 lock → runtime 應該重用,+0 MB)")
    ...
    step(8, "目標機:改變主意 —— 取消還沒套用的更新(--clear-pending)")
    ...
    step(9, "目標機:重新武裝那一版(--set-pending)—— 取消不等於判它死刑")
    ...
    step(10, "目標機:沒有可退回的版本時,--rollback 要「明講並失敗」,不能假裝成功")
    ...
    step(11, "目標機:GC 在繁中主控台(cp950)不能崩潰")
